...
use crate::settings::{
    ...
    clear_zrok_enable_token_environment, config_dir_override, effective_workspace_path,
    launch_env, mcp_url, redact_secrets, save_settings, settings_revision, AppPaths, Settings,
};
...
use crate::readiness::{real_probe_fn, ContextProvider, ProbeContext, ReadinessScheduler, ReadinessSnapshot};
...
fn download_file(url: &str, dest: &Path, deadline: Duration) -> Result<(), AppError> {
    ...
    loop {
        if start.elapsed() >= deadline {
            child.kill()?;
            let _ = child.wait();
            return Err(AppError::new("download_timeout", "Download timed out".to_string()));
        }
        ...
        match child.try_wait() {
            ...
            Err(e) => {
                let _ = child.kill();
                return Err(AppError::new("download_failed", format!("Download error: {e}")));
            }
        }
    }
}
...
fn extract_archive(archive: &Path, dest: &Path) -> Result<(), AppError> {
    ...
    fs::create_dir_all(dest).map_err(|e| {
        AppError::new("extraction_failed", format!("Failed to create extraction dir: {e}"))
    })?;
...
fn find_github_asset(
    ...
    let mut url = format!("{}/repos/{}/releases/tags/{}", 
    ...
    Err(AppError::new(
        "asset_not_found",
        format!("No asset matching '{}' found in release {}", asset_pattern, release_tag),
    ))
}
...
fn download_and_verify_component(
    ...
    fs::create_dir_all(dest_dir).map_err(|e| {
        AppError::new("download_failed", format!("Failed to create dest dir: {e}"))
    })?;
...
fn main() {
    ...
    for component in &manifest.components {
        println!("Downloading component: {} v{}", component.name, component.version);
        ...
        match result {
            Ok(archive_path) => {
                ...
                let verified_path = verified_dir.join(
    ...
    let status = ManifestStatus {
        ...
        components_verified: manifest.components
            .iter()
            .filter(|c| c.verified)
            .count() as u32,
    };
    ...
    println!("\nInstallation complete. Status written to: {:?}", status_path);
}
